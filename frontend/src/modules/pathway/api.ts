...
export const fetchCurrentPathwayPlan = async (): Promise<PathwayOverviewResponse> => {
  return (await api.get('/pathway/plan/current/')) as PathwayOverviewResponse
}

export const generatePathwayPlan = async (cycleDays = 7): Promise<PathwayOverviewResponse> => {
  return (await api.post('/pathway/plan/generate/', { cycle_days: cycleDays })) as PathwayOverviewResponse
}

export const startPathwayGeneration = async (cycleDays = 7): Promise<PathwayGenerationResponse> => {
  return (await api.post('/pathway/plan/generate-async/', { cycle_days: cycleDays })) as PathwayGenerationResponse
}

export const fetchPathwayGenerationStatus = async (jobId: number): Promise<PathwayGenerationResponse> => {
  return (await api.get(`/pathway/plan/generation-status/${jobId}/`)) as PathwayGenerationResponse
}

export const fetchLatestPathwayGeneration = async (): Promise<PathwayGenerationResponse> => {
  return (await api.get('/pathway/plan/generation-latest/')) as PathwayGenerationResponse
}
...
export const completePathwayTask = async (
  ...
  return (await api.post(`/pathway/tasks/${taskId}/complete/`, { status })) as PathwayTaskCompleteResponse
}

export const fetchPathwayEffect = async (): Promise<PathwayEffectResponse> => {
  return (await api.get('/pathway/effect/')) as PathwayEffectResponse
}
