...
class InterviewRoundAudioUploadView(APIView):
    ...
    @swagger_auto_schema(
        ...
        security=[{'Bearer': []}],
        responses={
            200: openapi.Response('获取成功', InterviewRoundAudioUploadResponseSerializer),
    ...
    def get(self, request, interview_id, round_id):
        ...
        round_obj = InterviewRound.objects.filter(id=round_id, interview=interview).first()
        if not round_obj:
    ...
    @swagger_auto_schema(
        ...
        security=[{'Bearer': []}],
        responses={
            201: openapi.Response('上传成功', InterviewRoundAudioUploadResponseSerializer),
            200: openapi.Response('覆盖成功', InterviewRoundAudioUploadResponseSerializer),
    ...
    def post(self, request, interview_id, round_id):
        ...
        try:
            ...
            return APIResponse.error(message='上传失败', code=500, errors={'detail': str(exc)})

        existing_audio = getattr(round_obj, 'audio', None)
        if existing_audio and existing_audio.file_key and existing_audio.file_key != saved_key:
            try:
